...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        self.train = json.load(open(train_file))
        ...
        for uid, train_items in self.train.items():
    ...
    def inter_matrix(self):
        ...
        for uid, train_items in self.train.items():
            if len(train_items) ==0:
                continue
            uid = int(uid)
            user.extend([uid] * len(train_items))
            item.extend(train_items)            
        data = np.ones(len(user))
        mat = coo_matrix((data, (user, item)), shape=(self.n_users, self.n_items))    
        return mat
    ...
    def i_i_matrix(self):
        ...
        for i in range(len(user)):
            if user[i] in u_i_dic:
                u_i_dic[user[i]].append(item[i])
            else:
                u_i_dic[user[i]] = [item[i]]
        i_i_dict ={}
        for i in range(len(user)):
            if item[i] in i_i_dict:
                i_i_dict[item[i]].extend(u_i_dic[user[i]])
            else:
                i_i_dict[item[i]] = (u_i_dic[user[i]])
        item_n=[]
        item1=[]
        for iid,item in i_i_dict.items():
            if len(train_items) ==0:
                continue
            iid = int(iid)
            ...
            if len(item) >50:
                item =item[:50]
            item1.extend([iid]*len(item))
            item_n.extend(item)

        data = np.ones(len(item1)) 
        i_i_mat = coo_matrix((data, (item1, item_n)), shape=(self.n_items, self.n_items)) 
        return  i_i_mat
